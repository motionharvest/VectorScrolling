(function(scope, factory) {
	...
	return function RenderFarm(options) {
		...
		return {
			...
			addPiggy: function(elem, options) {
				...
				onScrollCallbacks.push(function() {
					...
					if (pWhenStart < wTop + (wHeight * options.start.is) && pWhenEnd > wTop + (wHeight * options.end.is) && !active) {
					...
					} else if ((pWhenStart > wTop + (wHeight * options.start.is) || pWhenEnd < wTop + (wHeight * options.end.is)) && active) {
